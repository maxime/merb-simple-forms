...
describe <%= controller_name %>, "index action" do
  before :each do
    @parent = mock('<%= parent %>')
    <%= parent_class_name(parent) %>.stub!(:get).and_return(@parent)
  ...
  it "should have a route from /<%= plural_parent(parent) %>/1/<%= plural_name %> GET" do
    request_to('/<%= plural_parent(parent) %>/1/<%= plural_name %>', :get).should route_to(<%= controller_name %>, :index)
  end
  ...
  it "should get the <%= parent %> from the database" do
    <%= parent_class_name(parent) %>.should_receive(:get).with("1").and_return(@parent)
    do_index
  end
  
  it "should raise NotFound if the <%= parent %> isn't found" do
    <%= parent_class_name(parent) %>.stub!(:get).and_return(nil)
  ...
  it "should assign the <%= parent %> parent to the view" do
    do_index.assigns(:parent).should == @parent
  end  
end
...
describe <%= controller_name %>, "show action" do
  ...
  it "should have a route from /<%= plural_parent(parent) %>/1/<%= plural_name %>/2 GET" do
    request_to('/<%= plural_parent(parent) %>/1/<%= plural_name %>/2', :get).should route_to(<%= controller_name %>, :show)
  end
  ...
  it "should assign the <%= parent %> parent to the view" do
    do_show.assigns(:parent).should == @parent
  end  
...
describe <%= controller_name %>, "new action" do
  ...
  it "should have a route from /<%= plural_parent(parent) %>/1/<%= plural_name %>/new GET" do
    request_to("/<%= plural_parent(parent) %>/1/<%= plural_name %>/new", :get).should route_to(<%= controller_name %>, :new)
  end
  ...
  it "should assign the <%= parent %> parent to the view" do
    do_new.assigns(:parent).should == @parent
  end  
...
describe <%= controller_name %>, "edit action" do
  ...
  it "should have a route from /<%= plural_parent(parent) %>/1/<%= plural_name %>/2/edit GET" do
    request_to("/<%= plural_parent(parent) %>/1/<%= plural_name %>/2/edit", :get).should route_to(<%= controller_name %>, :edit)
  end
  ...
  it "should assign the <%= parent %> parent to the view" do
    do_edit.assigns(:parent).should == @parent
  end  
...
describe <%= controller_name %>, 'create action' do
  before :each do
    @parent = <%= parent_class_name(parent) %>.new
    @parent.stub!(:id).and_return("1")
    <%= parent_class_name(parent) %>.stub!(:get).with("1").and_return(@parent)
  ...
  it "should have a route from /<%= plural_parent(parent) %>/1/<%= plural_name %> POST" do
    request_to("/<%= plural_parent(parent) %>/1/<%= plural_name %>", :post).should route_to(<%= controller_name %>, :create)
  end
  ...
  it "should assign the <%= parent %> parent to the view" do
    do_create.assigns(:parent).should == @parent
  end  
end

describe <%= controller_name %>, 'update' do
  before :each do
    @parent = <%= parent_class_name(parent) %>.new(:id => "1")
    <%= parent_class_name(parent) %>.stub!(:get).with("1").and_return(@parent)
  ...
  it "should have a route from /<%= plural_parent(parent) %>/1/<%= plural_name %>/2 PUT" do
    request_to("/<%= plural_parent(parent) %>/1/<%= plural_name %>/2", :put).should route_to(<%= controller_name %>, :update)
  end
  ...
  it "should redirect to the <%= human_name.downcase %> if successful" do
    do_update.should redirect_to("/<%= plural_parent(parent) %>/1/<%= plural_name %>/2")
  end
  ...
  it "should assign the <%= parent %> parent to the view" do
    do_update.assigns(:parent).should == @parent
  end  
end
...
describe <%= controller_name %>, 'destroy' do
  ...
  it "should have a route from /<%= plural_parent(parent) %>/1/<%= plural_name %>/2 DELETE" do
    request_to("/<%= plural_parent(parent) %>/1/<%= plural_name %>/2", :delete).should route_to(<%= controller_name %>, :destroy)
  end
  ...
  it "should redirect to the <%= human_name.downcase %>s list if destroyed" do
    do_delete.should redirect_to("/<%= plural_parent(parent) %>/1/<%= plural_name %>")
  end
