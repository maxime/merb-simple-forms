...
describe "<%= plural_name %>/index, <%= parent %> as parent" do
  ...
  it "should display the <%= plural_human_name %> and <%= Extlib::Inflection.humanize(parent).pluralize %> title" do
    @body.should have_tag(:h1) {|h1| h1.should contain("<%= Extlib::Inflection.humanize(parent).pluralize %>")}    
    @body.should have_tag(:h1) {|h1| h1.should contain("<%= plural_human_name %>")}
  end
